...
export class ListMatterComponent implements OnInit {
 ...
 consola(){
  console.log(this.getMatters())
 }
  ngOnInit(): void {
    this.getMatters();
  }
  getMatters() {
    return this.subjectHttpService.findAll().subscribe((response) => {
      this.subjects = response;     
    });
  }  
  deleteMatter(id:SubjectModel['id']) {
    return this.subjectHttpService.remove(id).subscribe((response) => {
      this.subjects = this.subjects.filter(product => product.id != id);
